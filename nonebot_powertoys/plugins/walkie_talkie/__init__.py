...
from nonebot_plugin_saa import Text, Image, Mention, TargetQQGroup, MessageFactory
from nonebot.adapters.onebot.v11 import Bot, Message, MessageEvent, GroupMessageEvent

from nonebot_powertoys.utils import is_number, get_message_img
...
async def _(bot: Bot, event: MessageEvent, arg: Message = CommandArg()):
    ...
    if not text and not img_msg:
        ...
        await dialogue.send(Text(f"您的话已发送至管理员！\n======\n{text}") + img_msg, at_sender=True)
...
async def _(bot: Bot, event: MessageEvent, arg: Message = CommandArg()):
    ...
    for img in get_message_img(event.json()):
        text += Image(img)
    if group_id:
        target_group = TargetQQGroup(group_id=group_id)
        if user_id:
            msg_builder = MessageFactory([Text("\n管理员回复\n" + text), Mention(user_id=str(user_id))])
